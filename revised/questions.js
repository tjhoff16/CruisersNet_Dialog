...
function newsletterTask(qNumber, qSnippet, qAnswer) {
  ...
  switch (qAnswer) {
    ...
    case 'Want': // Wants to receive newsletter - Sign Them Up but check again in 45 days to make sure
      ...
      //need to change to open new div
      // if (typeof HTMLDialogElement === 'function') {
      //   jQuery(newsletterDiv).dialog({
      //     resizable: false,
      //     autoOpen: false,
      //     show: {
      //       effect: "fade",
      ...
      //     height: 300,
      //   });
      //   jQuery(newsletterDiv).html(html);
      //   jQuery(newsletterDiv).closest('.ui-dialog').find('.ui-dialog-titlebar-close').hide();
      //   jQuery(newsletterDiv).dialog('open');
      // } else {
      //   //  Dialog is unavailable
      //   jQuery(newsletterDiv).html(html);
      //   jQuery(newsletterDiv).css("display", "block");
      // }
      break;
